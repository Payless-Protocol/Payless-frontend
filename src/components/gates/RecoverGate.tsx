...
export default function RecoverGate() {
  ...
  const confirmAndSubmit = async () => {
    ...
    try {
      ...
      setMessage(`✅ Device successfully unflagged! Tx Hash: ${txHash}`);
    ...
    <div className="p-6 max-w-md mx-auto bg-neutral-900 rounded-xl shadow-md space-y-4 text-white border border-neutral-700">
      ...
      {showConfirm && (
        <div className="mt-4 p-4 bg-neutral-800 border border-neutral-600 rounded-lg">
          <h3 className="text-lg font-semibold text-white mb-2">Confirm Recover Device</h3>
          <p className="text-sm text-neutral-300 mb-4">
            You are about to recover/unflag device with IMEI: <span className="text-[#00F0FF] font-mono">{imei}</span>
          </p>
          <p className="text-xs text-neutral-400 mb-4">
            This action writes to the blockchain. Make sure you have the correct recovery phrase.
          </p>
          <div className="flex gap-3">
            ...
            >
              {loading ? "Processing..." : "Confirm & Recover"}
            </button>
          </div>
        </div>
      )}
